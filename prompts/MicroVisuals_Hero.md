...
- On `loadedmetadata`: call `video.play().catch(()=>{})` then start the capture loop.
...
- Variables: `index = 0`, `direction = 1`, `last = performance.now()`, `interval = 1000/30`.
- In a `requestAnimationFrame(render)` loop: if `now - last >= interval`, draw `frames[index]`, advance `index += direction`. When `index >= frames.length - 1`, clamp and flip to `-1`. When `index <= 0`, clamp and flip to `+1`.
