...
### Navigation (top)
- Left: "Lumora" in white, italic, `text-xl` (`sm:text-2xl`), Instrument Serif
...
- Right (mobile): A `.liquid-glass` rounded hamburger button using Lucide `Menu`/`X` icons with a crossfade rotation animation (300ms). Menu icon rotates out 90deg and scales to 75%; X icon rotates in from -90deg
...
- Easing: `cubic-bezier(0.4,0,0.2,1)`, duration 500ms

### Hero Content (centered, below nav)
- **Badge**: `.liquid-glass` rounded-full pill — "Over 10,000 minds already finding their clarity"
...
- **Email Input**: `.liquid-glass` rounded-full pill containing a text input ("Your Best Email") and a solid white "Get Early Access" button. `max-w-[320px]` on mobile, `sm:max-w-sm`
- **Video Switcher**: Row of 4 text buttons (Golden Hour, Still Water, Deep Woods, Quiet Dawn). Active button has solid color + bottom border. Inactive buttons 50% opacity, hover to 80%

### Dark Mode for "Deep Woods" (3rd video, index 2)
When the 3rd video is active, all hero content (badge, heading, subtext, input, video switcher) transitions to dark color `#182C41` with 700ms duration. Navbar and bottom stats remain white always.

### Bottom Stats
Pushed to bottom via `flex-1` spacer. Row of stats separated by `|` dividers (hidden on mobile):
- "60+ Deep Sessions"
- "12,000+ Creators"
...
- On click: if not already active and not mid-transition, set new active video and start 1000ms cooldown (matching CSS crossfade duration)
...
Black background prevents flash before videos load. Single viewport-height section, no scroll.
