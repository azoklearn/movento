...
- Boucle `requestAnimationFrame` séparée avec lerp : `currentTime += (targetTime - currentTime) * k`, facteur `k = Math.min(0.35, 0.1 + Math.abs(diff) * 0.03)` (~0.1 en scroll normal, renforcé sur les grands sauts de scrollbar).
  - Si `video.readyState < 2` OU `video.seeking` : ne PAS faire avancer le lerp, juste re-planifier la frame suivante et attendre (ne jamais empiler un seek sur un seek en cours — c'est ce qui fige l'image ; et ne jamais laisser la variable interne converger pendant que la vidéo ne peut pas être positionnée, sinon elle reste figée au chargement).
  - La boucle s'arrête quand `|diff| < 0.001` ; elle est relancée par `onUpdate` si éteinte (garder l'id rAF à null quand stoppée).
