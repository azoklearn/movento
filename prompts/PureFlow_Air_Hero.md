...
3. **Reveal layer (spotlight)** — a hidden `<canvas>` sized to `window.innerWidth` x `window.innerHeight`. Each frame (in a `useEffect` that runs on every render of `RevealLayer`):
   * Clear canvas.
